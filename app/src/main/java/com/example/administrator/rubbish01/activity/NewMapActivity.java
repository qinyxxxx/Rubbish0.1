            ...
            if (amapLocation != null) {
                if (amapLocation.getErrorCode() == 0) {
                    ...
                    for(int i=0;i<list.size();i++) {
                        ...
                        aMap.addMarker(maker2);
                        AMap.OnMarkerClickListener markerClickListener = new AMap.OnMarkerClickListener() {
                            @Override
                            public boolean onMarkerClick(Marker marker) {
                                Intent intent=null;
                                if((marker.getPosition().latitude==(lat+0.0001))&&(marker.getPosition().longitude ==(lon-0.0001))){
                                    intent=new Intent(NewMapActivity.this,Demo1Activity.class );}
                                if((marker.getPosition().latitude==(lat+0.0001))&&(marker.getPosition().longitude ==(lon+0.0001))){
                                    intent=new Intent(NewMapActivity.this,Demo2Activity.class );}
                                if((marker.getPosition().latitude==(lat-0.0001))&&(marker.getPosition().longitude ==(lon-0.0001))){
                                    intent=new Intent(NewMapActivity.this,Demo3Activity.class );}
                                if((marker.getPosition().latitude==(lat-0.0001))&&(marker.getPosition().longitude ==(lon+0.0001))){
                                    intent=new Intent(NewMapActivity.this,Demo4Activity.class );}
